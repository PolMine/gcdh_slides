...
- Substantive research interests
  ...
  - cp. DFG-Projekt (2019-2021): "The populist challenge in parliament" (in cooperation with Christian Strecker, Marcel Lewandowsky, Jochen Müller)
...
## Menu for today {.smaller}

- Combining distant and close reading is an unfulfilled promise. Software often inhibits combining both  perspectives. How to implement workflows for coding and annotating textual data? Introducing the *polmineR* R package (and a few brothers and sisters)
...
- NoSQL / Lucene / Elasticsearch: Potential alternatives?
...
* *performance*: if its slow, interaction with the data would suffer
...
- Once R and RStudio is installed, *polmineR* can be installed from GitHub (*RcppCWB* will be installed automatically).

```{r installation, eval = FALSE, message = FALSE}
install.packages("polmineR")
```

- Get GermaParl (corpus of plenary debates in the German Bundestag).
...
## polmineR - basic vocabulary

- subcorpora: *partition()*, *subset()*

- counting: *hits()*, *count()*, *dispersion()* (vgl.: *size()*)

- term-document-matrices: *as.TermDocumentMatrix()*

- keywords / feature extraction: *features()*
...
- old workflow to create subcorpora / partitions
...
```{r show_topic_vocab, echo = TRUE, message = FALSE}
...
```{r highlighted_vocab, cache = FALSE}
...
```{r activate_migparl, echo = FALSE, message = FALSE}
use("MigParl")
```

- Prepared in the MigTex Project ("Textressourcen für die Migrations- und Integrationsforschung", Funding: BMFSFJ)

- Preparation of all plenary debates in the regional parliaments using the "Framework for Parsing Plenary Protocols" (*frappp*-package)
...
<img src="img/maas_kind.png" height = "50%" width = "50%"/>


## "Wuchern der Rhizome" | (Joachim Scharloth)

<img src="img/wuchern_rhizome.png" height = "75%" width = "75%"/>
...
afd_rds_file <- "/Users/andreasblaette/Lab/gitlab/gradget_slides/data/afd_cooc.rds"
...
rds_file <- "/Users/andreasblaette/Lab/gitlab/gradget_slides/data/afd_cooc_features.rds"
...
- How to handle the complexity and lay the foundations for intersubjectivity?
